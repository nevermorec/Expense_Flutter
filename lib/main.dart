...
void main() async {
  ...
  runApp(const MyApp());
  _loadInitialData(); // Moved after runApp
}

Future<void> _loadInitialData() async {
  try {
    await fetchAndStoreExpenses();
    appState.notifyDataUpdated(); // Notify UI of data update
...
Future<void> fetchAndStoreExpenses() async {
  try {
    ...
    final response = await http.get(Uri.parse(
        'https://741096681c.azurewebsites.net/api/get_expense/${appState.familyId}?code=pJQTZnTB45OGUtJrKYCkn3_XukHiAVgCgEYPLPcVlCDWAzFuU8lAVQ%3D%3D'));
    ...
    if (response.statusCode == 200) {
      ...
      final existingExpenses = existingExpensesJson.map((e) => Expense.fromJson(e)).toList();
      ...
      await prefs.setStringList('expenses', mergedExpenses.values.map((e) => e.toJson()).toList());
      appState.notifyDataUpdated(); // Notify UI of data update
    } else {
